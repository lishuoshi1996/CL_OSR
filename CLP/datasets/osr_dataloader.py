...
from datasets.data_aug import contrastive_train_transform
...
class MNIST_OSR(object):
    def __init__(self, known, dataroot='./data/mnist', use_gpu=True, num_workers=8, batch_size=128, img_size=32):
        ...
        train_contrastive_set = MNIST_Filter(root=dataroot, train=True, download=True,
                                               transform=ThreeCropTransform(contrastive_train_transform(32)))
        train_contrastive_set.__Filter__(known=self.known)

        self.train_contrastive_loader = torch.utils.data.DataLoader(
            train_contrastive_set, batch_size=batch_size, shuffle=True,
...
class CIFAR10_OSR(object):
    def __init__(self, known, dataroot='./data/cifar10', use_gpu=True, num_workers=8, batch_size=128, img_size=32):
        ...
        train_contrastive_set = CIFAR10_Filter(root=dataroot, train=True, download=True, transform=ThreeCropTransform(contrastive_train_transform(32)))
        train_contrastive_set.__Filter__(known=self.known)
...
class SVHN_OSR(object):
    def __init__(self, known, dataroot='./data/svhn', use_gpu=True, num_workers=8, batch_size=128, img_size=32):
        ...
        train_contrastive_set = SVHN_Filter(root=dataroot, split='train', download=True, transform=ThreeCropTransform(contrastive_train_transform(32)))
        train_contrastive_set.__Filter__(known=self.known)
...
class Tiny_ImageNet_OSR(object):
    def __init__(self, known, dataroot='./data/tiny_imagenet', use_gpu=True, num_workers=8, batch_size=128,
        ...
        train_contrastive_set = Tiny_ImageNet_Filter(os.path.join(dataroot, 'tiny-imagenet-200', 'train'), ThreeCropTransform(contrastive_train_transform(64)))
        train_contrastive_set.__Filter__(known=self.known)
