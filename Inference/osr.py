...
from models.models_MultiKD import classifier, classifier_teacher, classifier32ABN
...
parser.add_argument('--dataroot', type=str, default='/home/feiran/fangsongyu/ARPL-master/data')
...
parser.add_argument('--loss', type=str, default='PrototypeLoss')
...
def main_worker(options):
    ...
    if options['cs']:
        ...
        net = classifier(num_classes=options['num_classes'])
        net_teacher = classifier_teacher()
        feat_dim = 128

        if options['dataset'] == 'cifar100':
            net_teacher.load_state_dict(torch.load(os.path.join(options['outf'], 'Pretrain_models', f"{options['dataset']}_{options['out_num']}", 'checkpoints', f"SupCL+AE_{options['item']}.pth")), strict=False)
            net.load_state_dict(torch.load(os.path.join(options['outf'], 'models', f"{options['dataset']}_{options['out_num']}", 'checkpoints', f"classifier_{options['loss']}_{options['item']}_{options['cs']}_.pth")), strict=False)
        else:
            net_teacher.load_state_dict(torch.load(os.path.join(options['outf'], 'Pretrain_models', options['dataset'], 'checkpoints',f"SupCL+AE_{options['item']}.pth")), strict=False)
            net.load_state_dict(torch.load(os.path.join(options['outf'], 'models', options['dataset'], 'checkpoints', f"classifier_{options['loss']}_{options['item']}_{options['cs']}_.pth")), strict=False)
    ...
    if options['eval']:
        ...
        results = eval(net, net_teacher, criterion, testloader, outloader, epoch=None, **options)
...
if __name__ == '__main__':
    ...
    for i in range(len(splits[options['dataset']])):
        ...
        dir_name = '{}_{}_{}'.format(options['model'], options['loss'], 'rejection_score')
