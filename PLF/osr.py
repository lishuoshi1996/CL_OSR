...
from core.train import train,train_cs
...
def main_worker(options):
    ...
    if options['cs']:
        ...
        if options['dataset'] == 'cifar100':
            net.load_state_dict(torch.load(os.path.join(options['outf'], 'Pretrain_models', f"{options['dataset']}_{options['out_num']}", 'checkpoints', f"SupCL+AE_{options['item']}.pth")), strict=False)
        else:
            net.load_state_dict(torch.load(os.path.join(options['outf'], 'Pretrain_models', options['dataset'], 'checkpoints', f"SupCL+AE_{options['item']}.pth")), strict=False)
    ...
    if options['eval']:
        ...
        results = eval(net, criterion, testloader, outloader, epoch=None, **options)
