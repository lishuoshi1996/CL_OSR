...
class classifier(nn.Module):
    ...
    def forward(self, x, return_feature=False):
        ...
        x = self.conv9(x)
        x = self.bn9(x)
        feature3 = nn.LeakyReLU(0.2)(x)
        
        x = self.avgpool(feature3)
        x = torch.flatten(x, 1)
        out = self.fc(x)
        if return_feature:
            return x, out, feature1, feature2, feature3
        else:
            return out


# Teacher model
class classifier_teacher(torch.nn.Module):
